...
Four dimensions, 0/1/2 each:
  spec_adherence, robustness, task_quality, economy
plus a boolean `violation` flag for the forbidden failures (character break,
real-world leak, or mathematically wrong teaching).

Scenarios may carry an `answer_key` (the correct numeric result); the judge is
told to force task_quality=0 on contradiction.

Usage:
    python eval/judge.py "student msg" "model reply" --category lesson --answer-key "6 leagues"
"""
...
from src.npc.prompts import judge_system, judge_user, load_world_bible  # noqa: E402

DIMENSIONS = ["spec_adherence", "robustness", "task_quality", "economy"]
...
_world_bible_cache: str | None = None
...
def judge_one(
    ...
    global _world_bible_cache
    if _world_bible_cache is None:
        _world_bible_cache = load_world_bible()

    raw = teacher.chat(
        judge_user(student_message, model_reply, category, answer_key),
        system=judge_system(_world_bible_cache),
        temperature=0.0,
        max_tokens=400,
        json_mode=True,
    )
...
def main() -> None:
    ...
    ap.add_argument("--category", default="lesson")
